...
def solicitar():
    # Solicité al usuario el número de escalones
...
while opcion != 0:
    opcion = solicitar()  # Recibo el número de escalones
    ...
    if opcion == 0:
        ...
        for i in range(1, opcion + 1):
            # Ajusté los espacios para que haya dos espacios entre cada escalón
            print(" " * (opcion - i) * 2 + "_|")  # Los espacios entre escalones son ahora de 2
    elif opcion < 0:
        # Si el número es negativo, la escalera será descendente
        for i in range(opcion, 0):
            # Para los escalones descendentes puse 2 espacios entre cada uno
            print(" " * (-i) * 2 + "|_")  # El espacio entre cada escalón lo multipliqué por 2
    else:
        print("Opción inválida")  # En caso de que no se ingrese un número válido
